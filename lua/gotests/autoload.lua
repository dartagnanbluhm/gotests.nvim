...
function M.tests(first, last, opts)
	local bin = opts.gotests_bin
	...
	local tmplDir = ""
	if opts.gotests_template_dir ~= "" then
		tmplDir = "-template_dir " .. shellescape(opts.gotests_template_dir)
	end
	local file = vim.fn.expand("%")
	local out = vim.fn.system(
		bin .. options .. " -only " .. shellescape(funcMatch) .. " " .. tmplDir .. " " .. shellescape(file)
	)

	vim.notify(out, vim.log.levels.INFO, { title = "gotests" })
end

function M.alltests(opts)
	local bin = opts.gotests_bin
	if fn.executable(bin) == nil then
		vim.notify("gotests binary not found.", vim.log.levels.ERROR, {
			title = "gotests.nvim",
	...
	local file = vim.fn.expand("%")
	local out = vim.fn.system(bin .. options .. " -all " .. tmplDir .. " " .. shellescape(file))
	vim.notify(out, vim.log.levels.INFO, { title = "gotests" })
end
